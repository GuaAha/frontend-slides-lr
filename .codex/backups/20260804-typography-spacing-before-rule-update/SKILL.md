...
- 将每段编写的文字准确映射到一个语言区域层级：主标题、副标题、标签、描述或免责声明。大型证据数字归入主标题层级/副标题层级，中文最大 `75px`/`45px`；一页唯一的核心证据数字可使用主标题层级，同页的次要或辅助证据数字使用副标题层级。其他语言区域使用对应的主标题/副标题字号。大型证据数字不构成第六层级。
...
- 引用或注释的指引数字，包括来源中的 `【1】`、`【2】`、`【3】` 及同类编号，使用对应语言区域的免责声明层级字号；不得继承相邻标题、正文或数据的字号。渲染时移除 `【` 和 `】`，只显示原编号数字。将显示后的编号放入独立的文字叶节点，来源台账保留带括号的原文。
- Q&A 中，`Q` 及其问题使用正文字号，即对应语言区域的描述层级；`A` 及其回答使用注释字号，即对应语言区域的免责声明层级。中文分别为 `30px` 和 `15px`。Q&A 的专项映射优先于卡片内副标题或正文统一使用描述层级的规则。
...
- 检查卡片内副标题和正文、引用或注释指引数字以及 Q&A 的计算字号，确认它们分别遵守步骤 3 的专项映射。引用编号仍包含 `【` 或 `】` 时验证失败。
