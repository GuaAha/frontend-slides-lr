...
def load_source() -> dict:
    ...
    if (
        ...
        raise ValueError("the fixed safe area must be top 120px / right 60px / bottom 60px / left 60px and Q&A group gap 60px")
    typography = data["typography"]
    if typography.get("line_height_percent") != 100:
        raise ValueError("the fixed typography requires line_height_percent = 100")
    if typography.get("annotation_line_height_px") != 0:
        raise ValueError("annotation_line_height_px must be 0")
...
:root {{
  ...
  --brand-annotation-line-height: {typography['annotation_line_height_px']}px;
...
def render_rules(data: dict) -> str:
    ...
    lines = [
        ...
        *(
        ...
        f"- All ordinary authored text leaves use **{typography['line_height_percent']}%** line height; annotation explanatory text uses **{typography['annotation_line_height_px']}px** line height as a scoped exception.",
        "- Every authored text run maps to one of the five locale levels; large evidence numerals use headline or subheadline size and remain one unified size on the same slide: 2 groups prefer 75px, 3 groups prefer 45px, and 4 or more groups force 45px.",
        ...
        "- Citation marker digits keep disclaimer size and ordinary 100% line height; brackets are removed for display and the marker may not form a line by itself. Annotation explanatory text alone uses 0px line height.",
        f"- Content blocks use **{data['spacing']['content_gap_px']}px** flow gaps; Q&A groups use **{data['spacing']['qa_group_gap_px']}px** from the end of one answer to the next question. Do not use `margin-top: auto`, `justify-content: space-between`, or equivalent empty-space distribution for authored content.",
