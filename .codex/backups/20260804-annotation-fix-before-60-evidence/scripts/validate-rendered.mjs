...
const typographyContract = {
  ...
  annotationLineHeightPx: brandSource.typography.annotation_line_height_px,
    ...
    const diagnostics = await page.evaluate(({ activeIndex, expectedHeight, typographyContract, safeAreaContract }) => {
      ...
      for (const element of textElements) {
        ...
        if (style.lineHeight === 'normal') {
          ...
          const expectedLineHeight = isAnnotationText
            ? typographyContract.annotationLineHeightPx
      ...
      for (const marker of slide.querySelectorAll('[data-citation-marker]')) {
        ...
        if (!previousText && !nextText) {
          issues.push({
            kind: 'citation-line',
            message: 'citation marker digits may not form a line by themselves',
          });
        }
      }
      ...
      if (evidenceMetrics.length >= 2) {
        const preferredMetricSize = evidenceMetrics.length === 2 ? 75 : 45;
