...
- `xx%用户认可` 等组合型内容视为一个不可拆分的语义单元，不得在数字、百分号和后续说明之间拆行或拆组。
...
- 应用 `brand/source.json` 中的语言区域字体规则。五级字号表如下；通用文字叶节点使用 `100%` 行高，注释说明文本使用 `0px` 行高这一已批准的组件级例外：
...
- 将每段编写的文字准确映射到一个语言区域层级：主标题、副标题、标签、描述或免责声明。大型证据数字归入主标题层级/副标题层级，中文最大 `75px`/`45px`；2 组优先使用 `75px`，3 组优先使用 `45px`，4 组及以上强制使用 `45px`。只要该页采用其中一个字号，所有大型证据数字必须统一使用该字号。其他语言区域使用对应的主标题/副标题字号。大型证据数字不构成第六层级。
...
- 引用或注释的指引数字，包括来源中的 `【1】`、`【2】`、`【3】` 及同类编号，使用对应语言区域的免责声明层级字号；不得继承相邻标题、正文或数据的字号。渲染时移除 `【` 和 `】`，只显示原编号数字；显示后的编号仍使用 `100%` 行高，且不得单独成行。来源台账保留带括号的原文。
...
- 文字叶节点是直接承载可见文字、且其内部不再包含其他文字角色的最末级 HTML 元素，例如标题、段落、标签、指标数字、上标和问答答案。除注释说明文本外，对每个文字叶节点应用对应语言区域的字距令牌和 `100%` 行高。注释说明文本（脚注、来源说明和免责声明正文）使用 `0px` 行高；注释指引数字保留免责声明字号和 `100%` 行高。不得使用浏览器的 `normal`、正向功能标签字距或未批准的组件专属行高覆盖。
...
- 检查卡片内副标题和正文、引用或注释指引数字以及 Q&A 的计算字号，确认它们分别遵守步骤 3 的专项映射；大型证据数字按同页组数选择并统一字号。引用编号仍包含 `【` 或 `】`、注释说明文本未使用 `0px` 行高或注释数字单独成行时验证失败。
- 检查 `xx%用户认可` 等组合型内容没有被拆组，内容块之间使用 `40px`，Q&A 组间使用 `60px`，且不存在用 `margin-top: auto` 或 `justify-content: space-between` 吸收剩余高度的规则。
