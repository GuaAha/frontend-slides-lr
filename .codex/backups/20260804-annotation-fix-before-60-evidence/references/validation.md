...
7. Verify citation markers use the locale disclaimer size, contain the original number, render without `【` or `】`, keep ordinary `100%` line height, and never form a line by themselves.
...
9. Every authored text leaf uses one of the five type sizes defined for its locale. The large-evidence count rule applies: large metrics and proof numerals use the locale headline or subheadline level and do not receive an exception above the locale headline size; two groups prefer headline, three groups prefer subheadline, four or more groups force subheadline, and all large metrics on one slide use one unified size.
10. Every ordinary authored text leaf resolves to exactly 100% line height and the locale letter spacing from `brand/source.json`. The annotation explanatory text (footnotes, source notes, and disclaimer prose) is the approved component-level exception and resolves to 0px line height; citation marker digits retain disclaimer size and 100% line height. Browser `normal`, positive utility-label tracking, and other component-specific overrides fail validation.
...
15. No visible line may contain only one Han character or one Han character followed by a punctuation mark. Citation marker digits may not be alone on a line, and combinations such as `xx%用户认可` are atomic. Fix the semantic break, text width, or layout without shrinking the type.
