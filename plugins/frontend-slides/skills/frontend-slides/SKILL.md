...
- 拆行必须依据语义短语和完整句意进行。不得拆开固定词组、数字与单位或引用标记；任何可见行不得只剩一个汉字。出现孤字时，调整语义断点、文字容器宽度或排版结构，不得通过缩小字号解决。
...
- 检查截图中的 KV、单图切片、其他非满版图片、三张场景图和全部中文拆行。KV 图片位不是满版底图时验证失败；单图既不是满版底图，也不满足固定宽 `630px` 且高度不低于 `870px` 时验证失败；出现左图右文、纵向卡片横向三列或单字成行时验证失败。
