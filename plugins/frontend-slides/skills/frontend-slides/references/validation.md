...
6. In placeholder mode, use authored placeholder layers that match the final image-frame geometry and no content images. Every placeholder must carry `data-image-placeholder`, use the neutral fill `#909090`, and contain a `data-image-placeholder-label` with the exact structural text `图片占位`. This label is an explicit placeholder marker, not source copy, and must not be used as a product, evidence, or image caption. KV placeholders default to a full-bleed `750 × 1320` background. For single-image slides, use either full bleed or a `630px`-wide frame at least `870px` high.
...
12. Verify KV media or placeholders use a full-bleed `750 × 1320` background by default while authored text remains inside the safe area. Placeholder layers must compute to `rgb(144, 144, 144)` and visibly contain the exact label `图片占位`.
