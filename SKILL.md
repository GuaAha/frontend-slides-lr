...
将 `templates/` 中的七个条目视为同级的设计风格和叙事结构基线。其来源只用于追溯，不代表优先级或能力等级。每个模板的 `preview.md` 和 `design.md` 负责定义该模板的配色；使用其中的构图、节奏、证据结构、组件语法、配色和应做或不应做的指导，但不得覆盖 `brand/source.json` 中的字体、徽标、资产、间距、形状、固定宽度和切片高度模式等非颜色品牌规范。

## 不可协商的不变规则

- 每张切片的宽度都使用精确的 `750px` CSS 像素值。KV 页固定为 `1320px` 高；非 KV 切片根据内容高度灵活设定，同时遵守安全上下边距。KV 指封面、主视觉页或来源明确标记为 KV 的切片；其他切片均标记为 `content`。
- 将所有编写的文字放在安全区域内：上边距 `120px`、右边距 `60px`、下边距 `60px`、左边距 `60px`；每张幻灯片的标题放在 `y=120px`。非 KV 切片的安全区下边界随该切片的明确高度计算。
- 直接写入 `750px` 宽度和每张切片的明确高度，不得根据宽高比推断，也不支持其他画布宽度。KV 写入 `1320px` 高度；非 KV 切片以最下方非出血内容的下边界加 `60px` 下安全边距确定高度并向上取整为整数，不得以无内容的额外留白任意加高。
- 内容块之间使用 `40px` 间距；禁止用 `margin-top:auto`（含带空格写法）、`justify-content: space-between` 或同类自动分配空白的规则来吸收剩余高度。非 KV 高度必须由最低内容下边界加 `60px` 得出。
...
- 从所选模板 `design.md` 已声明的配色中确定一个演示文稿级高亮色变量。所有被设计为高亮用途的文字、数字、分隔线、边框和色块都使用该变量；不得为不同高亮元素生成色阶、透明变体或第二个强调色。正文色、普通背景色和非高亮结构色不属于高亮色。
...
- 每个 `.slide` 固定为 `750px` 宽，并写入 `data-slide-kind="kv|content"` 与正整数 `data-slide-height`。KV 页的 `data-slide-height` 固定为 `1320`；非 KV 页根据内容和上下安全边距写入所需高度。每页唯一的主标题使用一个 `h1`，或在不能使用 `h1` 时使用一个 `data-type-level="headline"` 节点。运行时按当前切片高度整体缩放舞台，不得引起内部重排。
...
- 在 Playwright 可用时运行 `node scripts/validate-rendered.mjs <deck.html> --screenshots <directory>`。该命令以 `750px` 固定宽度、KV `1320px` 固定高度和各非 KV 页声明的内容高度渲染幻灯片，并报告边界、溢出、重叠、资产缺失和字体失败。修复单页时使用 `node scripts/validate-rendered.mjs <deck.html> --slide <页码> --screenshots <directory>` 只复验对应切片；页码从 `1` 开始。
...
- PDF 页面保持 `750px` 宽；KV 页保持 `1320px` 高，非 KV 页保留各自声明的内容高度。不得提供紧凑、横向或其他宽度模式。
...
| `viewport-base.css` | 生成的固定 `750px` 宽、按当前切片高度缩放的舞台 CSS |
...
| `scripts/export-pdf.sh` | 固定宽度、保留各切片高度的 PDF 导出工具 |
