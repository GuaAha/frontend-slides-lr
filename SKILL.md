...
- 用户选择占位符时，在运行清单中记录 `image_mode: placeholder`，并使用与最终图像位相同尺寸和位置的自定义占位层生成预览和完整演示文稿。占位符必须遵守当前切片的图片几何规则，不得把方形作为通用默认值。不得在这些图层中添加虚构的可见标签；只有当已批准的内容来源中存在完全一致的文字时，才能添加占位文字。
...
- 展示大纲和图像映射。占位符模式映射无文字、且与最终图片位几何一致的占位区域：KV 默认映射为满版底图；单图切片只映射为满版底图或 `630px` 宽、至少 `870px` 高的非满版图片。CSS 视觉模式说明抽象 CSS/SVG 视觉元素的作用，不得将其伪装为产品或证据图像。生成视觉预览前获得确认。
...
- 遵守已记录的图像模式和图片几何。占位符模式下，占位层复用最终图片位的尺寸和位置，不得引入搜索、推断或之前发现的图像。CSS 视觉模式下，视觉效果保持自包含和抽象；用户提供或授权模式下，只使用已记录的原图和矩形裁切派生图。KV 图片位默认满版；单图切片只使用满版底图或 `630px` 宽、至少 `870px` 高的非满版图片。
